...
`.github/workflows/pages.yml` を同梱しています。リポジトリの **Settings → Pages →
Source = "GitHub Actions"** に設定すると、`main` の `app/` が自動で公開されます。
URL は `https://<ユーザー名>.github.io/<リポジトリ名>/` です。

### ⚡ 自動編集（バックエンド方式）`server/`
アプリの ⚙️ に **バックエンドURL** を入れると、2つが有効になります:
- **署名アップロード**（未署名プリセット不要・APIシークレットはサーバーに留まる）
- **「自動で編集する」ボタン** … プロンプトを Claude が編集プランに変換 →
  ffmpeg で実行 → Cloudinary にアップ → 編集後リンクを返す

サーバーの立て方・必要なキーは [`server/README.md`](server/README.md) を参照。
Claude が出せるのはホワイトリスト化した操作（カット/テロップ/BGM/結合）だけで、
任意コマンド実行はできません。

> 補足: コピー／共有方式（プリセット方式）も引き続き使えます。バックエンドが
> 無い場合はそちらにフォールバックします。
